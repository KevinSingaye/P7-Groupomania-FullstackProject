  ...
  */
  ...
  ngOnInit(): void {
    ...
    this.utilisateurService.findOne(this.post.userId).subscribe((results) => {
      this.post = { ...this.post, photoUser: results.photo, nomUser: results.nom, emailUser: results.email }
    }, (error) => {
      console.error(error)
    })
    //this.displayButton = this.post.userId === this.userId || JSON.parse(moderateur);
    this.post = { ...this.post, displayButton: this.post.userId === this.userId || JSON.parse(moderateur) }
  }
  onReset(): void {
    this._id = undefined;
    this.description = '';
